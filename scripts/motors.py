...
from std_srvs.srv import Trigger, TriggerResponse #TriggerResponse make Trigger frame
from pimouse_ros.srv import TimedMotion # use motor control frame

class Motor():
	def __init__(self):
		# call set_power function. if power off is fail, system exit
	...
	def callback_tm(self, message):
		if not self.is_on:
			rospy.logerr("not enpowered at timed motion")
			return False
		dev = "/dev/rtmotor0"
		try:
			...
			rospy.logerr("cannot write to" + dev +"at timed motion")
			return False
		return True
